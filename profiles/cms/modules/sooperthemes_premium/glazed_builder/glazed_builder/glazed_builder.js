(function($) {
  ...
  function glazed_lang() {
    if (drupalSettings.glazedBuilder.glazedLanguage) {
      return drupalSettings.glazedBuilder.glazedLanguage;
    } else {
      return 'en';
    }
  }
   ...
   */
  if (!(drupalSettings.glazedBuilder.glazedBaseUrl)) {
    if ($('script[src*="glazed_builder.js"]').length > 0) {
      var glazed_builder_src = $('script[src*="glazed_builder.js"]').attr('src');
      drupalSettings.glazedBuilder.glazedBaseUrl = glazed_builder_src.slice(0, glazed_builder_src.indexOf('glazed_builder.js'));
    }
    else {
      if ($('script[src*="glazed_builder.min.js"]').length > 0) {
        var glazed_builder_src = $('script[src*="glazed_builder.min.js"]').attr('src');
        drupalSettings.glazedBuilder.glazedBaseUrl = glazed_builder_src.slice(0, glazed_builder_src.indexOf('glazed_builder.min.js'));
      }
    }
  }
  ...
  if (!drupalSettings.glazedBuilder.glazedEditor)
    drupalSettings.glazedBuilder.glazedEditor = false;
  ...
  */
  window.glazed_add_css = function(path, callback) {
    var url = drupalSettings.glazedBuilder.glazedBaseUrl + path;
  ...
  window.glazed_add_js = function(options) {
    if ('loaded' in options && options.loaded || 'glazed_exported' in window) {
      ...
      glazed_add_external_js(drupalSettings.glazedBuilder.glazedBaseUrl + options.path, 'callback' in options ? options.callback :
        function() {});
    }
  }
  ...
  function glazed_login(callback) {
    if (drupalSettings.glazedBuilder.glazedEditor) {
      callback(drupalSettings.glazedBuilder.glazedEditor);
      return;
    }
    ...
    if (drupalSettings.glazedBuilder.glazedAjaxUrl) {
      $.ajax({
        ...
        data: {
          ...
          type: type,
          name: name,
          lang: drupalSettings.glazedBuilder.glazedLanguage,
  ...
  function glazed_builder_get_cms_element_names(callback) {
    if (drupalSettings.glazedBuilder.cmsElementNames) {
      callback(drupalSettings.glazedBuilder.cmsElementNames);
      return;
    }
    if (window.glazed_online) {
      if (drupalSettings.glazedBuilder.glazedAjaxUrl) {
  ...
  function glazed_builder_load_cms_element(name, settings, container, data, callback) {
    if (drupalSettings.glazedBuilder.glazedAjaxUrl) {
      data.originalPath = drupalSettings.glazedBuilder.currentPath;
      $.ajax({
        type: 'POST',
        url: drupalSettings.glazedBuilder.glazedAjaxUrl,
        data: {
  ...
  function glazed_get_elements(callback) {
    if (drupalSettings.glazedBuilder && drupalSettings.glazedBuilder.glazedTemplateElements) {
      for (var name in drupalSettings.glazedBuilder.glazedTemplateElements) {
        drupalSettings.glazedBuilder.glazedTemplateElements[name].html = decodeURIComponent(drupalSettings.glazedBuilder.glazedTemplateElements[name].html);
      }
      callback(drupalSettings.glazedBuilder.glazedTemplateElements);
      return;
    }
  }
  ...
  function glazed_load_template(name, callback) {
    if (drupalSettings.glazedBuilder.glazedAjaxUrl) {
      ...
      var url = drupalSettings.glazedBuilder.glazedBaseUrl + '../images/glazed_templates/' + name;
      $.ajax({
        url: url,
        cache: false,
      }).done(function(data) {
  ...
  */
  ...
  mixin(glazedElements.prototype, {
    ...
    create_template_elements: function(elements) {
      ...
      for (var path in elements) {
        ...
        var element_baseurl = drupalSettings.glazedBuilder.glazedBaseUrl + '../glazed_elements/' + folders + '/';
                ...
                function template_element_urls(dom) {
                  function update_url(url) {
                    if (url.indexOf("glazed_elements") == 0) {
                      return drupalSettings.glazedBuilder.glazedBaseUrl + '../' + url;
                    }
                    else {
        ...
        mixin(TemplateElement.prototype, {
          ...
          render: function($) {
            var element = this;
            this.dom_element = $('<div class="az-element az-template ' + this.get_el_classes() + ' ' + this.get_content_classes() +
     ...
     // Setting timeout on sidebar script because drupalSettings takes some time to render and
      // glazedEditor was returning false when it's actually true. ~Jur 24-07-2017
      setTimeout(function(){
        $(function() {
          if (drupalSettings.glazedBuilder.glazedEditor && Object.keys(elements).length > 0 && glazed_containers.length > 0) {
            var menu = {
              '_': []
            };
            for (var path in elements) {
      ...
      for (var key in elements) {
        ...
        var object = {
          ...
          get_button_with_tag: function() {
            ...
            if ($.inArray(drupalSettings.glazedBuilder.viewsTags, this.base > -1)) {
              tag = drupalSettings.glazedBuilder.viewsTags[this.base];
            }
                  ...
                  $(element.dom_element).waypoint(function(direction) {
                    ...
                    glazed_builder_load_cms_element(element.instance, element.attrs['settings'], container.attrs['container'], data,
                      function(data) {
                        ...
                        Drupal.attachBehaviors($(element.dom_content_element)[0]);
                      }, true);
                  }, {
                    offset: '100%',
          ...
          render: function($) {
            this.dom_element = $('<div class="az-element az-cms-element ' + this.get_el_classes() + ' ' + this.get_content_classes() +
              '" style="' + this.attrs['style'] + '"></div>');
            this.dom_content_element = $('<div></div>').appendTo(this.dom_element);
        ...
        if (key.match("^view-")) {
          ...
          if (!(drupalSettings.glazedBuilder.cmsElementViewsSettings.hasOwnProperty(base))) {
            continue;
          }
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].view_display_type == 'block') {
          ...
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].title) {
            object.params.push(make_param_type({
              type: 'checkbox',
              heading: Drupal.t('Show title'),
              param_name: 'display_title',
          ...
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].pager.items_per_page)
            param_type_items.value = drupalSettings.glazedBuilder.cmsElementViewsSettings[base].pager.items_per_page;
          ...
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].pager.offset)
            param_type_offset.value = drupalSettings.glazedBuilder.cmsElementViewsSettings[base].pager.offset;
          object.params.push(make_param_type(param_type_offset));
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].contextual_filter)
            object.params.push(make_param_type({
              type: 'textfield',
          ...
          if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].use_fields) {
            var toggleFields = {
              type: 'checkboxes',
            ...
            for (var id in  drupalSettings.glazedBuilder.cmsElementViewsSettings[base].field_list) {
              var item = drupalSettings.glazedBuilder.cmsElementViewsSettings[base].field_list[id];
              toggleFields.value[id] = item;
            }
              if (drupalSettings.glazedBuilder.cmsElementViewsSettings[base].field_values !='')
                toggleFields.content = drupalSettings.glazedBuilder.cmsElementViewsSettings[base].field_values;
    ...
    make_elements_modal: function(container, pre_render_callback) {
      ...
      var tags = [];
      for (var key in drupalSettings.glazedBuilder.viewsTags) {
        if (tags.indexOf(drupalSettings.glazedBuilder.viewsTags[key]) == -1) {
          tags.push(drupalSettings.glazedBuilder.viewsTags[key]);
        }
      }
  ...
  function BaseElement(parent, position) {
    this.id = 'gb' + Math.random().toString(36).substr(2, 8);
    if (parent != null) {
      this.parent = parent;
  ...
  if ('glazed_classes' in window) {
    classes = drupalSettings.glazedBuilder.glazedClasses;
  }
  BaseElement.prototype = {
      ...
      if (drupalSettings.glazedBuilder.glazedEditor) {
        if (this.is_container && !this.has_content || (this instanceof UnknownElement)) {
          $(this.dom_content_element).sortable({
            ...
            placeholder: 'az-sortable-placeholder',
            // helper: function() { return $('<div class="btn-group btn-group-xs"><span class="btn btn-default glyphicon glyphicon-move">test</span></div>'); },
            forcePlaceholderSize: true,
            scrollSpeed: 100,
            //            tolerance: "pointer",
            //            distance: 1,
    ...
    get_content_classes: function() {
      ...
      if (this.attrs['shadow'] > 0) {
        classes = 'glazed-shadow-' + this.attrs['shadow'];
      }
    ...
    add_css: function(path, loaded, callback) {
      ...
      container.css[drupalSettings.glazedBuilder.glazedBaseUrl + path] = true;
      if (!loaded) {
    ...
    add_js_list: function(options) {
      var container = this.get_my_container();
      for (var i = 0; i < options.paths.length; i++) {
        container.js[drupalSettings.glazedBuilder.glazedBaseUrl + options.paths[i]] = true;
      }
      window.glazed_add_js_list(options);
    },
    add_js: function(options) {
      var container = this.get_my_container();
      container.js[drupalSettings.glazedBuilder.glazedBaseUrl + options.path] = true;
...
// Accepts 2 getBoundingClientRect objects
  ...
  function try_login() {
    if (!drupalSettings.glazedBuilder.glazedAjaxUrl)
      if (!drupalSettings.glazedBuilder.glazedEditor || window.glazed_online)
        delete drupalSettings.glazedBuilder.glazedEditor;
    glazed_login(function(data) {
      drupalSettings.glazedBuilder.glazedEditor = data;
      $(function() {
  ...
  function connect_container(dom_element) {
    if ($(dom_element).length > 0) {
      ...
      if (match || (html == '' && drupalSettings.glazedBuilder.glazedAjaxUrl)) {
  ...
  function glazed_load() {
    if (glazed_loaded)
      return;
    glazed_loaded = true;
     if (drupalSettings.glazedBuilder.disallowContainers);
    var glazedDisallowContainer = drupalSettings.glazedBuilder.disallowContainers;
        ...
        return this.each(function() {
          ...
          if (!container) {
            ...
            if ($(dom).find('> .az-container[data-az-type][data-az-name]').length > 0) {
              ...
              var type = 'textarea';
              var name = 'gb' + Math.random().toString(36).substr(2);
              if (window.glazed_online) {
  ...
  mixin(SectionElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div id="' + this.id + '" class="az-element az-section ' + this.get_el_classes() + ' ' + this.get_content_classes() +
        ' " style="' + this.attrs['style'] + '"></div>');
      ...
      switch (this.attrs['effect']) {
        case 'parallax':
          $(this.dom_element).attr('data-glazed-builder-libraries', 'waypoints');
        break;
        case 'youtube':
          $(this.dom_element).attr('data-glazed-builder-libraries', 'ytplayer');
        break;
        default:
          break;
      }
  ...
  mixin(RowElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div class="az-element az-row row ' + this.get_el_classes() + ' ' + this.get_content_classes() + '" style="' +
  ...
  mixin(ColumnElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div class="az-element az-ctnr az-column ' + this.get_el_classes() + ' ' + this.get_content_classes() + ' ' +
  ...
  mixin(TabsElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div class="az-element az-tabs tabbable ' + this.get_el_classes() + ' ' + this.get_content_classes() + this.attrs['az_dirrection'] + '" style="' + this.attrs[
        'style'] + '"></div>');
  ...
  mixin(TabElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div id="' + this.id + '" class="az-element az-ctnr az-tab tab-pane ' +
        this.get_el_classes() + ' ' + this.get_content_classes() + '" style="' + this.attrs['style'] + '"></div>');
      this.dom_content_element = this.dom_element;
  ...
  mixin(ToggleElement.prototype, {
    ...
    render: function($) {
      ...
      this.dom_element = $('<div class="az-element az-toggle panel ' + type + ' ' + this.get_el_classes() + ' ' + this.get_content_classes() + '" style="' + this.attrs['style'] + '"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#' +
  ...
  mixin(CarouselElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div id="' + this.id + '" class="az-element az-carousel ' + this.get_el_classes() + ' ' + this.get_content_classes() +
        '" style="' + this.attrs['style'] + '" data-glazed-builder-libraries="owlcarousel"></div>');
      this.dom_content_element = $('<div></div>').appendTo(this.dom_element);
  ...
  mixin(SlideElement.prototype, {
    ...
    render: function($) {
      ...
      if (this.parent.attrs['type'] != '')
        type = this.parent.attrs['type'];
      this.dom_element = $('<div class="az-element az-slide az-ctnr ' + this.get_el_classes() + ' ' + this.get_content_classes() + ' clearfix" style="' + this.attrs['style'] + '"></div>');
      this.dom_content_element = this.dom_element;
  ...
  mixin(LayersElement.prototype, {
    ...
    render: function($) {
      this.dom_element = $('<div class="az-element az-layers ' + this.get_el_classes() + ' ' + this.get_content_classes() + '" style="' + this.attrs[
  ...
  mixin(ContainerElement.prototype, {
    ...
    get_empty: function() {
      var dom_element = '<div class="az-empty"><div class="top well"><strong>' + '  <h3 class="glazed-choose-layout">Choose a template</h3></strong>'
           + '<p class="lead">' + Drupal.t('Or add elements from the + button or sidebar.' + '</p></div>')
           + '<div class="top-right well"><h1>↗</h1>'
           + ' <span class="glyphicon glyphicon-save"></span>' +
        Drupal.t(' Don\'t forget to save your work') + '</div></div>';
    ...
    get_loader: function() {
      ...
      function get_javascript() {
        ...
        */
        ...
        // // javascript += "drupalSettings.glazedBuilder.glazedBaseUrl = '" + drupalSettings.glazedBuilder.glazedBaseUrl + "';\n";
        // // if (drupalSettings.glazedBuilder.glazedAjaxUrl)
        // //   javascript += "drupalSettings.glazedBuilder.glazedAjaxUrl = '" + toAbsoluteURL(drupalSettings.glazedBuilder.glazedAjaxUrl) + "';\n";
        // javascript +=
        //   "window.glazed_online = (window.location.protocol == 'http:' || window.location.protocol == 'https:');\n";
        ...
        // javascript += "drupalSettings.glazedBuilder.glazedEditor = false;\n";
      ...
      */
      ...
      var javascript = '';
      var url = '';
      if (drupalSettings.glazedBuilder.glazedDevelopment) {
        url = drupalSettings.glazedBuilder.glazedBaseUrl + 'glazed_frontend.js';
      }
      else {
        url = drupalSettings.glazedBuilder.glazedBaseUrl + 'glazed_frontend.min.js';
      }
    ...
    get_html: function() {
      ...
      $(dom).find('.ckeditor-inline').each(function() {
        var $this = $(this);
    ...
    attach: function() {
      ...
      $(window).bind('CKinlineAttach', function() {
        function ckeditor_add_inline_editor() {
          ...
          // we're initializing editors manually later on, ensuring the configuration is OK
